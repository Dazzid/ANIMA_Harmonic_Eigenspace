...
const createGridSketch = (p) => {
    ...
    class ChordMemoryGrid {
        ...
        drawCell(p, row, col) {
            ...
            // Choose color
            let fillColor;
            if (this.waitingForCell && isHovered) {
            ...
            if (isFilled) {
                ...
                const label = chord.nodeNumber !== undefined ? `#${chord.nodeNumber}` : `${row},${col}`;
        ...
        handleClick(p) {
            ...
            // Case 2: Recalling existing chord
            const chord = this.storage[row][col];
            if (chord) {
        ...
        storeChord(row, col, chordData) {
            ...
            console.log(`Stored chord:`, this.storage[row][col]);
        }
...
function setupGridToggleButton() {
    ...
    // gridToggleBtn.addEventListener('mouseenter', () => {
    //     gridToggleBtn.style.background = 'rgba(76, 175, 80, 1)';
    // });

    // gridToggleBtn.addEventListener('mouseleave', () => {
    //     const container = document.getElementById('grid-container');
    //     if (container.style.display === 'none') {
    //         gridToggleBtn.style.background = 'rgba(76, 175, 80, 0.8)';
    //     }
    // });
